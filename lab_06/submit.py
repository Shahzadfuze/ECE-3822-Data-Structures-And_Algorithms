...
def main():
    ...
    # get student username: eg tux12345
    username  = get_username()

    # load student list from file
    # studentlist is dictionary, key is tux12345, value is real name
    studentlist = get_student_list()

    # augment the username
    # eg tux12345 -> obeid_tux12345
    student_lastname = studentlist[username].split(",")[0].lower()
    username_augmented = student_lastname + "_" + username
...
def rsync_files(target_dir):
    ...
    cmd_rsync  = "rsync -rv --chmod=D755,F744 --chown=:ece_3822"  + " "       
                                                            # basic command, force grp to ece_3822
                                                            # include only these files explicitly
    cmd_rsync += "--include=""results.txt"""    + " "
    cmd_rsync += "--include=""comb.py"""       + " "
